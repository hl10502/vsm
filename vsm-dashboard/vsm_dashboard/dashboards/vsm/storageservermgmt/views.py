...
from .forms import InstallServersForm
...
class InstallServersView(forms.ModalFormView):
    form_class = InstallServersForm
    template_name = 'vsm/storageservermgmt/installservers.html'
...
def install_server(request):
    ...
    print data
    try:
        code,ret = vsmapi.install_servers(request, data)
        print ret
        resp = dict(message="Install server successfully", status="OK", data="")
        resp = json.dumps(resp)
        return HttpResponse(resp)
    except:
        resp = dict(message="Install server failed", status="Bad", data="")
        resp = json.dumps(resp)
        return HttpResponse(resp)
