...
def rename_storage_pool(request, body):
    return vsmclient(request).vsms.rename_storage_pool(body)

def get_pool_by_id(request, body):
    return vsmclient(request).vsms.rename_storage_pool(body)
...
resp = None
#server api
def install_servers(request, servers=[]):
    return vsmclient(request).servers.install(servers)
...
def storage_pool1(request, pool_id):
    return vsmclient(request).vsms.storage_pool1(pool_id)
