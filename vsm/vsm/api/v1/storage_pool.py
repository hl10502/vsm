...
class StoragePoolController(wsgi.Controller):
    ...
    def rename(self, req, body=None):
        ...
        for key in ('pool_id', 'name'):
            if not key in pool_dict:
                msg = _("%s is not defined in pool" % key)
                raise exc.HTTPBadRequest(explanation=msg)
        pool_id = pool_dict['pool_id'].strip()
        name = pool_dict['name'].strip()
        body_info = {'pool_id': pool_id, 'name':name}
        return self.scheduler_api.rename_storage_pool(context, body_info)
    ...
    def show1(self, req):
        """ Return data about the given storage pool """
        context = req.environ['vsm.context']
        LOG.info('1req.GET.id=%s' % req.GET['id'])
        try:
            storage_pool = self.conductor_api.get_storage_pool1(context, req.GET['id'])
        except exception.NotFound:
            raise exc.HTTPNotFound()
